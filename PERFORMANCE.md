...
1. **End-to-end JavaScript:** an ordinary independent string-form puzzle enters each library's
   normal solve-once path and one solution is returned. Parsing and puzzle-specific setup are timed.
2. **Prepared JavaScript:** callers already hold each library's natural parsed representation. Only
   representation conversion is excluded; puzzle-specific solver topology and search-state
...
4. **Alternative runtimes:** Wasm, native addons, and native executables are reported separately
   from the main JavaScript table with their initialization and process boundaries stated.
...
- Wasm instantiation in the explicitly labelled steady-state Wasm group.
...
third-party corpora remain opt-in and are reported separately because their runtimes, initialization
costs, and licenses differ from the in-process JavaScript suite.

The Wasm group reports steady-state solving: module instantiation happens once before validation
and timing, while per-call string marshalling remains timed. It does not claim a cold-start rate.
The native competition uses one fresh, single-threaded process per complete canonical corpus pass
and includes process startup, corpus input, solving, and complete solution-file output in wall time.
Builds, corpus canonicalization, and exhaustive post-run validation happen outside timing. Its
ranked rate is total puzzles across all measured process runs divided by their total elapsed time.
Upstream self-reported internal timers remain diagnostic because their boundaries differ.
